...
var log = logging.Logger("modulesext")
...
type DealInfo struct {
	//RetrievalFile RetrievalFileInfo
...
var sizeOfDealInfo = int(unsafe.Sizeof(DealInfo{}))

// StorageClientOption allows custom configuration of a storage client
type StorageClientOption func(c *ClientEx)
...
func NewClientEx(
	...
	c.receiver = event.DataTransferEventReceiver{Sms: sync.Map{}, Dispatch: c.dispatch}
	...
	c.unsubDataTransfer = dataTransfer.SubscribeToEvents(event.DataTransferSubscriber(&c.receiver))

	err := dataTransfer.RegisterVoucherType(&requestvalidation.StorageDataTransferVoucher{}, requestvalidationex.NewUnifiedRequestValidatorEx(nil, &clientPullDeals{c}))
	if err != nil {
		return nil, err
	}
...
func (c *ClientEx) Stop() error {
	c.unsubDataTransfer()
	return c.statemachines.Stop(context.TODO())
}

// ListProviders queries chain state and returns active storage providers
func (c *ClientEx) ListProviders(ctx context.Context) (<-chan storagemarket.StorageProviderInfo, error) {
...
func (c *ClientEx) ListLocalDeals(ctx context.Context) ([]storagemarket.ClientDeal, error) {
...
func (c *ClientEx) GetLocalDeal(ctx context.Context, cid cid.Cid) (storagemarket.ClientDeal, error) {
	var out storagemarket.ClientDeal
...
func (c *ClientEx) GetAsk(ctx context.Context, info storagemarket.StorageProviderInfo) (*storagemarket.StorageAsk, error) {
	if len(info.Addrs) > 0 {
...
func (c *ClientEx) GetProviderDealState(ctx context.Context, proposalCid cid.Cid) (*storagemarket.ProviderDealState, error) {
...
func (c *ClientEx) ProposeStorageDeal(ctx context.Context, params storagemarket.ProposeStorageDealParams) (*storagemarket.ProposeStorageDealResult, error) {
	...
	num, _ := rand.Int(rand.Reader, bigmath.NewInt(9999999999))
	pcMin := big.Add(params.Collateral, abi.NewTokenAmount(num.Int64()))
	num, _ = rand.Int(rand.Reader, bigmath.NewInt(9999999999))
	clientCollateral := big.NewInt(num.Int64())
	...
	//buf, err := cborutil.Dump(&dealProposal)
	//if err != nil {
	//	return nil, err
	//}
	//generate a rand byte array and put filehash and fileflag te data to transfer to target miner
	strHash, err := c.getFileHashAndFlag(params.Data.Root, params.Info.PeerID)
	if err != nil {
		return nil, err
	}
	strLen := len(strHash) + 32
	data := make([]byte, strLen)
	rand.Read(data[:31])
	copy(data[32:], strHash)

	clientDealProposal := &marketactor.ClientDealProposal{
	...
	err = c.receiver.Begin(proposalNd.Cid(), deal)
	if err != nil {
	...
	s, err := c.net.NewDealStream(ctx, deal.Miner)
	if err != nil {
		return nil, xerrors.Errorf("ClientEventWriteProposalFailed %w", err)
	}

	c.net.TagPeer(deal.Miner, deal.ProposalCid.String())

	if err := s.WriteDealProposal(proposal); err != nil {
		return nil, xerrors.Errorf("ClientEventWriteProposalFailed %w", err)
	}

	resp, _, err := s.ReadDealResponse()
	if err != nil {
		return nil, xerrors.Errorf("ClientEventReadResponseFailed %w", err)
	}

	err = s.Close()
	if err != nil {
		return nil, xerrors.Errorf("ClientEventStreamCloseError %w", err)
	}

	if resp.Response.State != storagemarket.StorageDealWaitingForData {
		return nil, xerrors.Errorf("this proposal has been sent, StorageDealWaitingForData")
	}
	...
	_, err = c.dataTransfer.OpenPushDataChannel(ctx,
		deal.Miner,
	...
	if err != nil {
		return nil, xerrors.Errorf("dataTransfer.OpenPushDataChannel %w", err)
	}
...
func (c *ClientEx) getFileHashAndFlag(cid cid.Cid, peerId peer.ID) (string, error) {
	localCid := peer.ToCid(peerId)
	key := datastore.NewKey(cid.String() + localCid.String() + "_FileHashAndFlag")
	valbuf, err := c.Ds.Get(key)
	if err != nil {
		return "", xerrors.Errorf("DataStore get key % failed", key)
	}
...
//func (c *ClientEx) getStorageType(cid cid.Cid, peerId peer.ID) (string, error) {
//	localCid := peer.ToCid(peerId)
//	key := datastore.NewKey(cid.String() + localCid.String() + "_StorageType")
//	valbuf, err := c.Ds.Get(key)
//	if err != nil {
//		return "", xerrors.Errorf("DataStore get key % failed", key)
//	}
//	storageType := string(valbuf)
//	c.Ds.Delete(key)
...
func (c *ClientEx) GetPaymentEscrow(ctx context.Context, addr address.Address) (storagemarket.Balance, error) {
	tok, _, err := c.node.GetChainHead(ctx)
	if err != nil {
...
func (c *ClientEx) AddPaymentEscrow(ctx context.Context, addr address.Address, amount abi.TokenAmount) error {
...
func (c *ClientEx) restartDeals(ctx context.Context) error {
...
func (c *ClientEx) dispatch(eventName fsm.EventName, deal fsm.StateType) {
...
func (c *ClientEx) verifyStatusResponseSignature(ctx context.Context, miner address.Address, response network.DealStatusResponse, origBytes []byte) (bool, error) {
	tok, _, err := c.node.GetChainHead(ctx)
	if err != nil {
...
func (c *ClientEx) addMultiaddrs(ctx context.Context, providerAddr address.Address) error {
	tok, _, err := c.node.GetChainHead(ctx)
	if err != nil {
		return err
	}
...
func DealInfoStructToBytes(s *DealInfo) []byte {
	var x reflect.SliceHeader
	x.Len = sizeOfDealInfo
	x.Cap = sizeOfDealInfo
	x.Data = uintptr(unsafe.Pointer(s))
	return *(*[]byte)(unsafe.Pointer(&x))
}

func BytesToDealInfo(b []byte) *DealInfo {
	return (*DealInfo)(unsafe.Pointer(
		(*reflect.SliceHeader)(unsafe.Pointer(&b)).Data,
	))
}

func (c *ClientEx) CreateNewHead(ctx context.Context, oldproposal *retrievalmarket.DealProposal, params storagemarket.ProposeStorageDealParams, txhash string) (string, error) {
	num, _ := rand.Int(rand.Reader, bigmath.NewInt(9999999999))
	pcMin := big.Add(params.Collateral, abi.NewTokenAmount(num.Int64()))
	num, _ = rand.Int(rand.Reader, bigmath.NewInt(9999999999))
	clientCollateral := big.NewInt(num.Int64())
	...
	label, err := clientutils.LabelField(params.Data.Root)
	if err != nil {
		return "", xerrors.Errorf("creating label field in proposal: %w", err)
	}
	...
	log.Info("CreateNewHead Root:", params.Data.Root.String())
	//retrievaldeal := new(DealInfo)
	//retrievaldeal := DealInfo{RetrievalFileInfo{[]byte(txhash), true, params.Data.Root.Bytes()}}

	retrievaldeal := DealInfo{Txhash: txhash, Ishead: true, FileCid: params.Data.Root.String()}
	//retrievaldeal := DealInfo{"", true, ""}

	jretrievaldeal, err := json.Marshal(retrievaldeal)
	if err != nil {
		log.Error("json err")
	}
	...
	//err = enc.Encode(&retrievaldeal)
	//if err != nil {
	//	log.Error("Encode ", err)
	//}
	//retrievaldeal.RetrievalFile.Txhash = txhash
	//retrievaldeal.RetrievalFile.Ishead = true
	//retrievaldeal.RetrievalFile.FileCid = params.Data.Root.String()
	//retrievaldeal.Proposal = oldproposal
	log.Info("Send byte len :", len(jretrievaldeal))
	log.Info("Send byte  :", string(jretrievaldeal))

	clientDealProposal := &marketactor.ClientDealProposal{
		Proposal: dealProposal,
		ClientSignature: acrypto.Signature{
			Type: crypto.SigTypeBLS,
			Data: jretrievaldeal, //DealInfoStructToBytes(retrievaldeal),
		},
	}

	proposalNd, err := cborutil.AsIpld(clientDealProposal)
	if err != nil {
		return "", xerrors.Errorf("getting proposal node failed: %w", err)
	}
	...
	err = c.receiver.Begin(proposalNd.Cid(), deal)
	if err != nil {
		return "", xerrors.Errorf("setting up deal tracking: %w", err)
	}

	defer c.receiver.End(proposalNd.Cid())
	...
	s, err := c.net.NewDealStream(ctx, deal.Miner)
	if err != nil {
		return "", xerrors.Errorf("ClientEventWriteProposalFailed %w", err)
	}
	...
	resp, _, err := s.ReadDealResponse()
	if err != nil {
		return "", xerrors.Errorf("ClientEventReadResponseFailed %w", err)
	}

	err = s.Close()
	if err != nil {
		return "", xerrors.Errorf("ClientEventStreamCloseError %w", err)
	}

	if resp.Response.State != storagemarket.StorageDealWaitingForData {
		return "", xerrors.Errorf("this proposal has been sent, StorageDealWaitingForData")
	}

	return resp.Response.Message, err
}
