...
func (m *Sealing) getTicket(ctx statemachine.Context, sector SectorInfo) (abi.SealRandomness, abi.ChainEpoch, bool, error) {
	...
	backend := m.backend
	lastBlock, err := backend.BlockByNumber(context.Background(), rpc.LatestBlockNumber)
	if err != nil {
		return nil , 0, allocated, err
	}else {
		block, err := backend.BlockByNumber(context.Background(), rpc.BlockNumber(lastBlock.NumberU64() - 10))
		if err != nil {
			return nil , 0, allocated, err
		}
		randData := blake2b.Sum256((*block.Header()).Hash().Bytes())
		rand, _ := sealing2.DrawRandomness(randData[:], block.Number().Int64(), buf.Bytes(), int64(crypto.DomainSeparationTag_SealRandomness))
		return rand, abi.ChainEpoch(block.Number().Int64()), allocated, nil
	}
}
...
func (m *Sealing) preCommitParams(ctx statemachine.Context, sector SectorInfo) (*miner.SectorPreCommitInfo, big.Int, sealing.TipSetToken, error) {
	tok, height, err := m.Api.ChainHead(ctx.Context())
	if err != nil {
...
func (m *Sealing) handleWaitSeed(ctx statemachine.Context, sector SectorInfo) error {
	...
	//randHeight := policy.GetPreCommitChallengeDelay()

	//err = m.events.ChainAt(func(ectx context.Context, _ sealing.TipSetToken, curH abi.ChainEpoch) error {
	...
	//if err != nil {
	//	err = xerrors.Errorf("failed to get randomness for computing seal proof (ch %d; rh %d; tsk %x): %w", tok, randHeight, tok, err)
	...
	preiod := m.backend.ChainConfig().Bor.Period
	timeCtx , _ := context.WithTimeout(context.Background(), time.Duration(preiod * 10) * time.Second)
	select {
		case <- timeCtx.Done():
	}
	backend := m.backend
	blockLast, err := backend.BlockByNumber(context.Background(), rpc.LatestBlockNumber)
	if err != nil {
		_ = ctx.Send(SectorChainPreCommitFailed{error: err})
		return nil
	} else {
		// reduce 10
		block , _ := backend.BlockByNumber(context.Background(), rpc.BlockNumber(blockLast.Number().Int64() - 10))
		randData := blake2b.Sum256((*block.Header()).Hash().Bytes())
		rand2, _ := sealing2.DrawRandomness(randData[:], block.Number().Int64(), buf.Bytes(), int64(crypto.DomainSeparationTag_InteractiveSealChallengeSeed))
		_ = ctx.Send(SectorSeedReady{SeedValue: rand2, SeedEpoch: abi.ChainEpoch(block.Number().Int64())})
	}


	return nil
	//}, func(ctx context.Context, ts sealing.TipSetToken) error {
...
func (m *Sealing) handleSubmitCommit(ctx statemachine.Context, sector SectorInfo) error {
	...
	//submit to chain
	_, err = m.submitCommitToChain(sector)
	if err != nil {
		return ctx.Send(SectorCommitFailed{xerrors.Errorf("pushing message to chain: %w", err)})
	}
...
func (m *Sealing) submitCommitToChain(sector SectorInfo) (common.Hash, error) {
	ethBackend := m.backend
	...
	isReal := sector.SectorNewType == SectorStorage
	/*
	var cvotes []borcontracts.Cvote
	cvotes = append(cvotes, borcontracts.Cvote{
		SectorInx:     uint64(sector.SectorNumber),
		SealProofType: uint64(sector.SectorType),
		SealedCID:     sector.CommR.Bytes(),
		Proof:         sector.Proof,
	})
	cvotesBytes, _ := rlp.EncodeToBytes(cvotes)


	data, err := borcontracts.PowerCliObject.PackAddValidatorPowerAndProof(isReal, addr, new(bigext.Int).SetUint64(uint64(sector.SectorType)), cvotesBytes)
	if err != nil {
		return common.Hash{}, errors.New("pack data is err")
	}
	auth, err := borcontracts.PowerCliObject.GenerateAuthObj(privateKey, m.backend.ChainConfig().ChainID, addr, data)
	if err != nil {
		return common.Hash{}, err
	}

	txHash, err := borcontracts.PowerCliObject.AddValidatorPowerAndProofByClient(auth,isReal, addr, new(bigext.Int).SetUint64(uint64(sector.SectorType)), cvotesBytes)
	log.Info("submitCommitToChain tx :", txHash)
	if err != nil {
		return common.Hash{}, err
	}*/
	var storageSealInfos []w3fsStorageManager.StorageSealInfo
	storageSealInfos = append(storageSealInfos, w3fsStorageManager.StorageSealInfo{
		SealProofType : uint64(sector.SectorType),
		SectorNumber : uint64(sector.SectorNumber),
		TicketEpoch : uint64(sector.TicketEpoch),
		SeedEpoch : uint64(sector.SeedEpoch),
		SealedCID : sector.CommR.Bytes(),
		UnsealedCID : sector.CommD.Bytes(),
		Proof : sector.Proof,
	})
	cvotesBytes, _ := rlp.EncodeToBytes(storageSealInfos)
	data, err := w3fsStorageManager.GlobalStorageManagerClient.PackAddSealInfo(isReal, addr, cvotesBytes)
	if err != nil {
		return common.Hash{}, errors.New("pack data is err")
	}
	auth, err := w3fsStorageManager.GlobalStorageManagerClient.GenerateAuthObj(privateKey, m.backend.ChainConfig().ChainID, addr, data)
	if err != nil {
		return common.Hash{}, err
	}
	txHash, err := w3fsStorageManager.GlobalStorageManagerClient.AddSealInfo(auth, isReal, addr, cvotesBytes)
	log.Info("submitCommitToChain tx :", txHash)
	if err != nil {
		return common.Hash{}, err
	}
