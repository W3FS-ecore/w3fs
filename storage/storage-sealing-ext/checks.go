...
func checkPieces(ctx context.Context, maddr address.Address, si SectorInfo, api sealing.SealingAPI) error {
	...
	for i, p := range si.Pieces {
		...
		_, err := api.StateMarketStorageDealProposal(ctx, p.DealInfo.DealID, tok)
		if err != nil {
...
func checkPrecommit(ctx context.Context, maddr address.Address, si SectorInfo, tok sealing.TipSetToken, height abi.ChainEpoch, api sealing.SealingAPI) (err error) {
	...
	if err != nil {
		if err == sealing.ErrSectorAllocated {
...
func (m *Sealing) checkCommit(ctx context.Context, si SectorInfo, proof []byte, tok sealing.TipSetToken) (err error) {
	//if si.SeedEpoch == 0 {
	...
	backend := m.backend
	block, err := backend.BlockByNumber(context.Background(), rpc.BlockNumber(si.SeedEpoch))
	if err != nil {
	...
	seed, err := sealing2.DrawRandomness(randData[:], int64(si.SeedEpoch), buf.Bytes(), int64(crypto.DomainSeparationTag_InteractiveSealChallengeSeed))
	if err != nil {
		return &ErrApi{xerrors.Errorf("failed to get randomness for computing seal proof: %w", err)}
	}
