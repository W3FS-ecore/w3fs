...
type PreCommitBatcher struct {
	api       sealing.PreCommitBatcherApi
...
func NewPreCommitBatcher(mctx context.Context, maddr address.Address, api sealing.PreCommitBatcherApi, addrSel sealing.AddrSel, feeCfg config.MinerFeeConfig, getConfig sealing.GetSealingConfigFunc) *PreCommitBatcher {
	b := &PreCommitBatcher{
		api:       api,
		maddr:     maddr,
		mctx:      mctx,
...
func (b *PreCommitBatcher) processBatch(cfg sealiface.Config, tok sealing.TipSetToken, bf abi.TokenAmount, nv network.Version) ([]sealiface.PreCommitBatchRes, error) {
