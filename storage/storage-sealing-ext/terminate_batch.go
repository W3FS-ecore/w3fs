...
type TerminateBatcher struct {
	api       sealing.TerminateBatcherApi
	...
	todo map[sealing.SectorLocation]*bitfield.BitField // MinerSectorLocation -> BitField
...
func NewTerminationBatcher(mctx context.Context, maddr address.Address, api sealing.TerminateBatcherApi, addrSel sealing.AddrSel, feeCfg config.MinerFeeConfig, getConfig sealing.GetSealingConfigFunc) *TerminateBatcher {
	b := &TerminateBatcher{
		api:       api,
		maddr:     maddr,
		mctx:      mctx,
		...
		todo:    map[sealing.SectorLocation]*bitfield.BitField{},
		waiting: map[abi.SectorNumber][]chan cid.Cid{},
...
func (b *TerminateBatcher) processBatch(notif, after bool) (*cid.Cid, error) {
	...
	for _, t := range params.Terminations {
		delete(b.todo, sealing.SectorLocation{
			Deadline:  t.Deadline,
