...
func NewCommitBatcher(mctx context.Context, maddr address.Address, api sealing.CommitBatcherApi, addrSel sealing.AddrSel, feeCfg config.MinerFeeConfig, getConfig sealing.GetSealingConfigFunc, prov ffiwrapper.Prover) *CommitBatcher {
	b := &CommitBatcher{
		api:       api,
		maddr:     maddr,
		mctx:      mctx,
...
func (b *CommitBatcher) processSingle(cfg sealiface.Config, mi miner.MinerInfo, avail *abi.TokenAmount, sn abi.SectorNumber, info AggregateInput, tok sealing.TipSetToken) (cid.Cid, error) {
	enc := new(bytes.Buffer)
...
func (b *CommitBatcher) getSectorCollateral(sn abi.SectorNumber, tok sealing.TipSetToken) (abi.TokenAmount, error) {
