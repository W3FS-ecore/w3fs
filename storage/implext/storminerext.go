...
type StorageMinerAPI struct {
	...
	// Miner / storage
	Miner       *storageext.MinerExt        `optional:"true"`
...
func (sm *StorageMinerAPI) SectorsList(context.Context) ([]abi.SectorNumber, error) {
	...
	for _, sector := range sectors {
		if sector.State == sealingext.UndefinedSectorState {
...
func (sm *StorageMinerAPI) SectorsListInStates(ctx context.Context, states []api.SectorState) ([]abi.SectorNumber, error) {
	filterStates := make(map[sealingext.SectorState]struct{})
	for _, state := range states {
		st := sealingext.SectorState(state)
		if _, ok := sealingext.ExistSectorStateList[st]; !ok {
			continue
		}
...
func (sm *StorageMinerAPI) SectorsUpdate(ctx context.Context, id abi.SectorNumber, state api.SectorState) error {
	return sm.Miner.ForceSectorState(ctx, id, sealingext.SectorState(state))
}
