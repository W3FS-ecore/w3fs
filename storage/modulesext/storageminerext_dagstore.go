...
func NewMinerAPI(ctx context.Context, r repo.LockedRepo, pieceStore dtypes.ProviderPieceStore, sa retrievalmarket.SectorAccessor) (mdagstore.MinerAPI, error) {
	...
	//})
	//if err := <-ready; err != nil {
	//	return mountApi, fmt.Errorf("aborting dagstore start; piecestore failed to start: %s", err)
	//}
...
// passing to markets.
func DAGStore(r repo.LockedRepo, minerAPI mdagstore.MinerAPI) (*dagstore.DAGStore, *mdagstore.Wrapper, error) {
	cfg, err := extractDAGStoreConfig(r)
	if err != nil {
		return nil, nil, err
	}
