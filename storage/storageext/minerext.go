...
import (
	...
	specsstorage "github.com/filecoin-project/specs-storage/storage"
)
...
type SealingStateEvt struct {
	...
	From         sealingext.SectorState
	After        sealingext.SectorState
	Error        string
}
...
func NewMinerExt(api fullNodeFilteredAPI,
	maddr address.Address,
	ds datastore.Batching,
	sealer sectorstorage.SectorManager,
	sc sealing.SectorIDCounter,
	...
	backend ethapi.Backend) (*MinerExt, error) {
	m := &MinerExt{
		api:     api,
		feeCfg:  feeCfg,
		sealer:  sealer,
...
func (m *MinerExt) Run(ctx context.Context) error {
	...
	evtsAdapter := storage.NewEventsAdapter(evts)
	...
	adaptedAPI := NewSealingAPIAdapterExt(m.api)
	...
	as := func(ctx context.Context, mi miner.MinerInfo, use api.AddrUse, goodFunds, minFunds abi.TokenAmount) (address.Address, abi.TokenAmount, error) {
		...
		if err != nil {
			log.Errorf("failed parsing actor flag value (%q): %w", act, err)
			return address.Undef, big.Zero(), xerrors.Errorf("failed parsing actor flag value (%q): %w", act, err)
		}
		return a, big.NewInt(1000000), nil
	}

	// Instantiate the sealing FSM.
	m.sealing = sealingext.New(ctx, adaptedAPI, m.feeCfg, evtsAdapter, m.maddr, m.ds, m.sealer, m.sc, m.verif, m.prover, &pcp, cfg, m.handleSealingNotifications, as, m.backend)
...
func (m *MinerExt) handleSealingNotifications(before, after sealingext.SectorInfo) {
...
func (m *MinerExt) Stop(ctx context.Context) error {
	return m.sealing.Stop(ctx)
}

// runPreflightChecks verifies that preconditions to run the miner are satisfied.
func (m *MinerExt) runPreflightChecks(ctx context.Context) error {
...
type StorageWpp struct {
	prover   specsstorage.Prover
	verifier ffiwrapper.Verifier
	miner    abi.ActorID
	winnRpt  abi.RegisteredPoStProof
}

func NewWinningPoStProver(api v1api.FullNode, prover specsstorage.Prover, verifier ffiwrapper.Verifier, miner dtypes.MinerID) (*StorageWpp, error) {
