...
type ValidatorSealPowerSet struct {
	ValidatorSealPowers []*ValidatorSealPower `json:"validatorSealPowers"`
}
...
func NewValidatorSealPowerSet(valz []*ValidatorSealPower) *ValidatorSealPowerSet {
	vals := &ValidatorSealPowerSet{}
	vals.ValidatorSealPowers = valz
	return vals
}
...
func (valz *ValidatorSealPowerSet) Copy() *ValidatorSealPowerSet {
	if valz.ValidatorSealPowers == nil {
		return nil
	}
	valsCopy := make([]*ValidatorSealPower, len(valz.ValidatorSealPowers))
	for i, val := range valz.ValidatorSealPowers {
		valsCopy[i] = val.Copy()
	}
	return &ValidatorSealPowerSet{
		ValidatorSealPowers: valsCopy,
	}
}

func (vals *ValidatorSealPowerSet) hasAddressAndPower(address []byte) bool {
	idx := sort.Search(len(vals.ValidatorSealPowers), func(i int) bool {
		return bytes.Compare(address, vals.ValidatorSealPowers[i].Address.Bytes()) <= 0
	})
	return idx < len(vals.ValidatorSealPowers) && bytes.Equal(vals.ValidatorSealPowers[idx].Address.Bytes(), address) && vals.ValidatorSealPowers[idx].SealPower > 0
}


func (vals ValidatorSealPowerSet) hasTotalPower() bool {
	for _ , v := range vals.ValidatorSealPowers {
		if v.SealPower > 0 {
			return true
		}
	}
...
func (vals ValidatorSealPowerSet) getTotalPower() uint64 {
	var totalPower uint64
	for _ , v := range vals.ValidatorSealPowers {
		totalPower += v.SealPower
	}
	return totalPower
}

func (vals ValidatorSealPowerSet) getSignerPower(address []byte) uint64 {
	idx := sort.Search(len(vals.ValidatorSealPowers), func(i int) bool {
		return bytes.Compare(address, vals.ValidatorSealPowers[i].Address.Bytes()) <= 0
	})
	return vals.ValidatorSealPowers[idx].SealPower
}
