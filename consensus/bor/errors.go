...
type MismatchingValidatorsPowerError struct {
	Number             uint64
	ValidatorSetSnap   []byte
	ValidatorSetHeader []byte
}

func (e *MismatchingValidatorsPowerError) Error() string {
	return fmt.Sprintf(
		"Mismatching validators at block %d\nValidatorBytes from snapshot: 0x%x\nValidatorBytes in Header: 0x%x\n",
		e.Number,
		e.ValidatorSetSnap,
...
type ErrorNoWinningData struct {
	Number      uint64
	TotalPower  uint64
	SignerPower uint64
}

func (e *ErrorNoWinningData) Error() string {
	return fmt.Sprintf("Wrong no WinningData at block : %d, totalPower : %d, SignerPower : %d" , e.Number, e.TotalPower, e.SignerPower)
}


type ErrorCheckProof struct {
	Number      uint64
	Signer      common.Address
}
func (e *ErrorCheckProof) Error() string {
	return fmt.Sprintf("VerifyWinningPoSt is error, number is %d , signer is %s ", e.Number, e.Signer.String())
}
