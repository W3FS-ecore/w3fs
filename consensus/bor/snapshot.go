...
type Snapshot struct {
	...
	ValidatorSealPowerSet *ValidatorSealPowerSet `json:"validatorSealPowerSet"`
...
func newSnapshot(
	...
	validatorSealPowers []*ValidatorSealPower,
	ethAPI *ethapi.PublicBlockChainAPI,
) *Snapshot {
	snap := &Snapshot{
		config:       config,
		...
		ValidatorSealPowerSet: NewValidatorSealPowerSet(validatorSealPowers),
...
func (s *Snapshot) copy() *Snapshot {
	cpy := &Snapshot{
		...
		ValidatorSealPowerSet: s.ValidatorSealPowerSet.Copy(),
		Recents:      make(map[uint64]common.Address),
	}
...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		if number > 0 && (number+1)%s.config.Sprint == 0 {
			...
			if len(header.ExtraPower) == 0 {
				return nil, errors.New("header.ExtraPower is empty")
			}
			validatorPowerBytes := header.ExtraPower[:]
			newVazs, _ := ParseValidatorSealPower(validatorPowerBytes)
			snap.ValidatorSealPowerSet = NewValidatorSealPowerSet(newVazs)
