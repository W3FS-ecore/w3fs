...
type Validator struct {
	...
	StoragePower 	 int64			`json:"storagePower"`
	StorageSize  	 int64          `json:"storageSize"`
	StoragePromise 	 int64        	`json:"storagePromise"`
}
...
func (v *Validator) MiniPromiseVal() MiniPromiseVal {
	return MiniPromiseVal{
		Signer: v.Address,
		StoragePromise: uint64(v.StoragePromise),
	}
}
...
type MiniPromiseVal struct {
	Signer         common.Address `json:"signer"`
	StoragePromise uint64         `json:"storagePromise"`
}

type MinimalAddrVal struct {
	Signer      common.Address `json:"signer"`
}
...
func NewValidatorSealPower(address common.Address, sealPower uint64) *ValidatorSealPower {
	return &ValidatorSealPower{
		Address:   address,
		SealPower: sealPower,
	}
}

func ParseValidatorSealPower(validatorsBytes []byte) ([]*ValidatorSealPower, error) {
	if len(validatorsBytes) % 40 != 0 {
		return nil, errors.New("Invalid ValidatorSealPower bytes")
	}
	result := make([]*ValidatorSealPower, len(validatorsBytes)/40)
	for i := 0; i < len(validatorsBytes); i += 40 {
		address := make([]byte, 20)
		power := make([]byte, 20)
		copy(address, validatorsBytes[i:i+20])
		copy(power, validatorsBytes[i+20:i+40])
		result[i/40] = NewValidatorSealPower(common.BytesToAddress(address), big.NewInt(0).SetBytes(power).Uint64())
	}
...
type ValidatorSealPower struct {
	Address   common.Address `json:"signer"`
	SealPower uint64         `json:"sealPower"`
}


func (v *ValidatorSealPower) Copy() *ValidatorSealPower {
	vCopy := *v
	return &vCopy
}

func (v *ValidatorSealPower) HeaderBytes() []byte {
	result := make([]byte, 40)
	copy(result[:20], v.Address.Bytes())
	copy(result[20:], v.PowerBytes())
	return result
...
func (v *ValidatorSealPower) PowerBytes() []byte {
	powerBytes := big.NewInt(0).SetUint64(v.SealPower).Bytes()
	result := make([]byte, 20)
	copy(result[20-len(powerBytes):], powerBytes)
	return result
}

type ValidatorSealPowerByAddress []*ValidatorSealPower

func (valz ValidatorSealPowerByAddress) Len() int {
	return len(valz)
}

func (valz ValidatorSealPowerByAddress) Less(i, j int) bool {
	return bytes.Compare(valz[i].Address.Bytes(), valz[j].Address.Bytes()) == -1
}

func (valz ValidatorSealPowerByAddress) Swap(i, j int) {
	it := valz[i]
	valz[i] = valz[j]
	valz[j] = it
}
