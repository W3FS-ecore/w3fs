...
type Server struct {
	...
	//to prevent the mismatch node to connect
	NetworkID uint64
}
...
func (srv *Server) DiscV4() *discover.UDPv4 {
	return srv.ntab
}
...
func (srv *Server) setupLocalNode() error {
	...
	listenPortIndex := strings.LastIndex(srv.ListenAddr, ":")
	listenPort := srv.ListenAddr[listenPortIndex+1:]
	listenPortInt, err := strconv.ParseUint(listenPort, 10, 64)
	if err == nil {
		srv.ourHandshake = &protoHandshake{Version: baseProtocolVersion, Name: srv.Name, ID: pubkey[1:], ListenPort: listenPortInt, NetworkID: srv.NetworkID}
	} else {
		srv.ourHandshake = &protoHandshake{Version: baseProtocolVersion, Name: srv.Name, ID: pubkey[1:], NetworkID: srv.NetworkID}
	}
	srv.Protocols[0].NodeInfo()
...
func (srv *Server) setupConn(c *conn, flags connFlag, dialDest *enode.Node) error {
	...
	if srv.NetworkID != phs.NetworkID {
		clog.Trace("mismatch network id", "phsnetworkid", phs.NetworkID, "NetworkId", srv.NetworkID)
	...
	///put remote port into node
	enode.Set(c.node, enr.WithEntry("Bor_Remote_Port", phs.ListenPort))
	return nil
}
