...
var (
	...
	Ip4PortFlag = cli.IntFlag{
		Name:  "ip4.port",
		Usage: "P2p Ip4 Network listening port",
		Value: 30308,
	}
	Ip6PortFlag = cli.IntFlag{
		Name:  "ip6.port",
		Usage: "P2p Ip6 Network listening port",
		Value: 30309,
	}
	...
	GCacheForFileStoreDisableFlag = cli.BoolFlag{
		Name:  "gcache.file.disable",
		Usage: "gcache for file store disable (default: false)",
	}

	GCacheFileSizeFlag = cli.IntFlag{
		Name:  "gcache.file.size",
		Usage: "Whether to enable the cache of the specified size",
		Value: 10000,
	}

	GCacheForMemoryDisableFlag = cli.BoolFlag{
		Name:  "gcache.memory.disable",
		Usage: "gcache for memory disable (default: false)",
	}

	GCacheMemorySizeFlag = cli.IntFlag{
		Name:  "gcache.memory.size",
		Usage: "Whether to enable the cache of the specified size",
		Value: 10000,
	}
)
...
// setIp4Port creates p2p listening address string from set command
// line flags.
func setP2pPort(ctx *cli.Context, cfg *node.Config) {
	if ctx.GlobalIsSet(Ip4PortFlag.Name) {
		cfg.Ip4Port = fmt.Sprintf("%d", ctx.GlobalInt(Ip4PortFlag.Name))
	}
	if ctx.GlobalIsSet(Ip6PortFlag.Name) {
		cfg.Ip6Port = fmt.Sprintf("%d", ctx.GlobalInt(Ip6PortFlag.Name))
	}
}
