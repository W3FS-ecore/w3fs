...
type Config struct {
	...
	// gcache config
	GCacheForFileSotreDisable bool
	GCacheFileSize            int

	GCacheForMemoryDisable bool
	GCacheMemorySize       int
}
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, ethConfig *Config, db ethdb.Database, blockchainAPI *ethapi.PublicBlockChainAPI) consensus.Engine {
	...
	if chainConfig.Bor != nil {
		return bor.New(chainConfig, db, blockchainAPI, ethConfig.HeimdallURL, ethConfig.WithoutHeimdall, stack.DataDir()+"/.w3fsminer")
	}
